...
import numpy as np
from model import ADPGE, Discriminator
...
def get_model(placeholders, num_features, num_nodes, features_nonzero,pri_attr,dim_attr):
    ...
    model = None
    model = ADPGE(placeholders, num_features, features_nonzero,pri_attr,dim_attr)
...
#                     model.attr3_logits,model.attr4_logits]
    pred_attrs = [model.attr0_logits,model.attr1_logits]
...
def update(model, opt, sess, adj_norm, adj_label, features, placeholders, adj):
    ...
    for j in range(5):
        _, reconstruct_loss = sess.run([opt.G_opt_op, opt.cost], feed_dict=feed_dict)
#         for m in range(10):
#             _, pri_loss = sess.run([opt.D_opt_op, opt.attr2_loss], feed_dict=feed_dict)
    ...
    return emb_long,emb_concat, avg_cost
