...
class OptimizerAE(object):
    def __init__(self, preds, labels, pos_weight, norm, d_real, d_fake,pred_attrs,attr_labels_list,sample_list):
        ...
        self.attr2_loss = tf.losses.softmax_cross_entropy(logits= tf.cast(pred_attrs[2],tf.float32),
                                                             onehot_labels=attr_labels_list[2],
                                                         reduction = tf.losses.Reduction.NONE)
        mask_attr = np.sum(attr_labels_list[2],axis = 1)
        self.attr2_loss = tf.multiply(self.attr2_loss,sample_list)
        self.attr2_loss = tf.reduce_mean(tf.multiply(self.attr2_loss,mask_attr))
        ...
        self.A_opt_op = self.A_optimizer.minimize(self.attr2_loss,var_list=pri_var)
